...
//and an array of scores, write a function that 
//returns a new array containing the names of all students whose average score is greater
// than or equal to 85.
function getTopStudents(students) {
  const studentAverages = student.map(student => {
    const averageScore = students.scores.reduce((sum, score) => sum + score, 0) / student.scores.length;
    return { name: student.name, averageScore };
  });
  const topStudents = studentAverages.filter(student => student.averageScore >= 85)
